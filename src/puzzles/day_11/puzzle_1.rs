...
fn find_most_active(monkeys: &Vec<Monkey>) -> u32 {
  ...
  for idx in 0..monkeys.len() {
    while !monkeys[idx].items.is_empty() {
      ...
      if let Some(mut item) = item_opt {
        let throw_idx = inspect_item(&mut item, &mut monkeys[idx]);
...
fn inspect_item(item: &mut Item, monkey: &mut Monkey) -> usize{
  // count inspection
  ...
  // test
  let condition = match monkey.throw_conditional.test {
    BooleanTest::Divisible(divisor) => item.worry % divisor == 0
  };
...
fn resolve(identifier: &WorryIdentifierNode, old: i32) -> i32 {
  match identifier {
    WorryIdentifierNode::Old => old,
    WorryIdentifierNode::Literal(val) => *val
  }
}
...
fn parse_starting_items(items_line: &str) -> ParseResult<VecDeque<Item>> {
  ...
  let starting_items: VecDeque<Item> = after.split(',').map(|i| Item { worry: i.trim().parse::<i32>().unwrap() }).collect();
  Ok(starting_items)
}
...
fn parse_test_line(test_line: &str, true_line: &str, false_line: &str) -> ThrowIf {
  ...
  let test = BooleanTest::Divisible(after.parse::<i32>().unwrap());
...
fn parse_identifier(identifier_token: &str) -> ParseResult<WorryIdentifierNode> {
  if identifier_token == "old" {
    ...
    let literal = identifier_token.parse::<i32>()?;
...
mod tests {
  ...
  Test: divisible by 13
    ...
    assert_eq!(parsed.throw_conditional, ThrowIf { test: BooleanTest::Divisible(13), if_true: 1, if_false: 3});
  }
  ...
  fn parse_identifier_literals() {
    ...
    assert_eq!(parse_identifier("-14"), Ok(WorryIdentifierNode::Literal(-14)));
